...
import {
    ...
    CalendarDays, CalendarRange, Target, X, History, Flame, Zap,
...
const PomodoroApp = () => {
    ...
    // Add these new state variables for manual max overrides
    const [manualMaxDailyStreak, setManualMaxDailyStreak] = useState(null);
    const [manualMaxSessionStreak, setManualMaxSessionStreak] = useState(null);
 ...
 */
    const handleTimerComplete = useCallback(() => {
        if (!isBreak) {
            ...
            if (mode !== 'stopwatch') {
                ...
                const sessionValue = mode === '25/5' ? 0.5 : 1.0;

                // Update temporary session counter and session streak
                setTempSessionCount(prev => prev + 1);
                setSessionStreak(prev => prev + sessionValue);

                // Update daily streak immediately when session completes
                setDailyStreak(prev => prev + sessionValue);
            } else {
                // For stopwatch mode, only count the session without affecting streaks
                setTempSessionCount(prev => prev + 1);
                // Don't modify daily or session streaks for stopwatch
            }
    ...
    }, [isBreak, mode, playFinishSound, playSessionStartSound]);
    ...
    // Reset daily streak at midnight
    useEffect(() => {
        const checkMidnight = () => {
            const now = new Date();
            const today = now.toISOString().split('T')[0];

            // Get last recorded date from streak history
            const lastEntry = streakHistory[streakHistory.length - 1];
            const lastRecordedDate = lastEntry?.date;

            // If it's a new day and we have a current streak
            if (lastRecordedDate !== today && dailyStreak > 0) {
                // Save yesterday's streak to history
                ...
                if (lastRecordedDate === yesterday) {
                    ...
                    setStreakHistory(prev => {
                        ...
                        newHistory[newHistory.length - 1] = { date: yesterday, streak: dailyStreak };
                        return newHistory;
                    });
                } else {
                    // Add new entry for yesterday
                    setStreakHistory(prev => [...prev, { date: yesterday, streak: dailyStreak }]);
                }
            ...
            // If we have no recorded date at all and it's a new session, don't reset
            // This handles the case where the app is started for the first time today
        };

        // Only check every minute, not immediately on mount
    ...
    }, [isRunning, isBreak, updateActiveTaskTime]);
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                if (data.streakHistory && Array.isArray(data.streakHistory)) setStreakHistory(data.streakHistory);
                if (data.manualMaxDailyStreak !== undefined) setManualMaxDailyStreak(data.manualMaxDailyStreak);
                if (data.manualMaxSessionStreak !== undefined) setManualMaxSessionStreak(data.manualMaxSessionStreak);
                ...
                if (todaySessions.length > 0) {
                    ...
                    setDailyStreak(Math.max(data.dailyStreak || 0, todayDailyStreak));
                } else {
 ...
 */
    const stopTimer = useCallback(() => {
        ...
        // ================================
        // DAILY STREAK IS NOW UPDATED IMMEDIATELY WHEN SESSIONS COMPLETE
        // ================================

        // Daily streak is already up to date from handleTimerComplete
        const newDailyStreak = dailyStreak;
        ...
        if (mode === 'stopwatch') {
            ...
            if (tempSessionCount > 0 || totalSessionMinutes > 0) {
                ...
                const newSession = {
                    ...
                    dailyStreak: newDailyStreak, // Use the updated daily streak
                    timestamp: new Date().toISOString(),
                    sessionType: mode,
                    taskBreakdown: taskBreakdownWithNames
                };
        ...
        // Reset session streak after a brief delay to ensure session is saved
        setTimeout(() => {
            setSessionStreak(0);
        }, 100);
        setTaskSessionTimes({});
        ...
        setSessionElapsedTime(0);
    }, [isRunning, updateActiveTaskTime, taskSessionTimes, tasks, mode, tempSessionCount, activeTaskId, taskSessionNames, dailyStreak, sessionStreak]);
    ...
    */
    ...
    const getMaxDailyStreakForPeriod = useCallback((period) => {
        ...
        filteredSessions.forEach(session => {
            ...
            if (session.sessionType === '25/5') {
                dailyStreaksByDay[date] += 0.5;
            } else if (session.sessionType === '50/10') {
                dailyStreaksByDay[date] += 1.0;
            }
    ...
    const getMaxSessionStreakForPeriod = useCallback((period) => {
        // If manual override is set, use that instead
        if (manualMaxSessionStreak !== null) {
    ...
    */
    ...
    }, [dailyGoal, tasks, completedTasks, timeHistory, taskHistory, dailyStreak, sessionStreak, streakHistory]);
    ...
    const importData = useCallback(async (event) => {
        ...
        try {
            ...
            if (window.confirm('This will replace all your current data. Are you sure you want to continue?')) {
                ...
                if (data.taskHistory && Array.isArray(data.taskHistory)) setTaskHistory(data.taskHistory);
                if (data.dailyStreak !== undefined) setDailyStreak(data.dailyStreak);
                if (data.sessionStreak !== undefined) setSessionStreak(data.sessionStreak);
                if (data.streakHistory && Array.isArray(data.streakHistory)) setStreakHistory(data.streakHistory);
                if (data.manualMaxDailyStreak !== undefined) setManualMaxDailyStreak(data.manualMaxDailyStreak);
                if (data.manualMaxSessionStreak !== undefined) setManualMaxSessionStreak(data.manualMaxSessionStreak);
    ...
    return (
        <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #1f2937 0%, #111827 100%)' }}>
            {/* REPLACE the old title bar section with this: */}
            <TitleBar />
            ...
            <div className="pt-8" style={{ background: 'linear-gradient(135deg, #1f2937 0%, #111827 100%)', minHeight: '100vh' }}>
                <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-4">
                    ...
                    {/* IMPORTANT: Update the positioning of your clock and date */}
                    {/* Digital Clock - Top Left (changed from top-6 to top-10) */}
                    ...
                    {/* Date Display - Top Right (changed from top-6 to top-10) */}
                                                ...
                                                <div className="grid grid-cols-2 gap-1 mb-1">
                                                    ...
                                                    <div className="text-center">
                                                        <div className="text-xs font-bold text-yellow-400/80 flex items-center justify-center gap-1">
                                                            ...
                                                            {mode === 'stopwatch' ? '—' : Math.round(sessionStreak * 10) / 10}
                                                        </div>
                                                ...
                                                <div className="relative bg-gradient-to-br from-gray-800/40 to-gray-900/40 backdrop-blur-md rounded-2xl border border-white/10 shadow-[0_20px_40px_rgba(0,0,0,0.3),inset_0_1px_2px_rgba(255,255,255,0.05)] p-6 hover:shadow-[0_25px_50px_rgba(0,0,0,0.4)] transition-all duration-500">
                                                    ...
                                                    <div className="flex items-center gap-3 mb-6">
                                                        ...
                                                        <div className="ml-auto px-3 py-1 bg-blue-500/20 border border-blue-500/30 rounded-full shadow-[0_0_12px_rgba(59,130,246,0.3)]">
                                                            <span className="text-sm font-medium text-blue-400">{Math.max(0, tasks.length - 1)}</span>
                                                        </div>
                                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
                                    ...
                                    <div className="bg-orange-900/30 p-4 rounded-lg text-center">
                                        <div className="text-2xl font-bold text-orange-300 flex items-center justify-center gap-1">
                                            🔥 {getMaxDailyStreakForPeriod(viewMode)}
                                        </div>
                                        <div className="text-sm text-orange-400">Daily Max ({viewMode})</div>
                                    </div>
                                    <div className="bg-red-900/30 p-4 rounded-lg text-center">
                                        <div className="text-2xl font-bold text-yellow-300 flex items-center justify-center gap-1">
                                            <Zap className="w-5 h-5" /> {getMaxSessionStreakForPeriod(viewMode)}
                                        </div>
                                        <div className="text-sm text-yellow-400">Session Max ({viewMode})</div>
                                    </div>
                                </div>
                                            ...
                                            <button
                                                onClick={() => {
                                                    if (window.confirm('Clear all work sessions? This will also reset your streak data.')) {
                                                        ...
                                                        setManualMaxDailyStreak(null);
                                                        setManualMaxSessionStreak(null);
                                                    }
                                                }}
